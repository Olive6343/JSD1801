package com.ldq.bms.book.dao.impl;
...
public class BookDAOImpl implements BookDAO {
	...
	public List<BookInfo> searchBooksByNameOrStatus(String bname, String status) {
		...
		try {
			...
			conn = DBUtil.getConnection();
			// 2.创建SQL语句
			String sql = "select * from book_info where name like ? and status liek ?";
			// 3.创建preparedStatmenet对象
			ps.setString(1, '%'+bname+'%');
			ps.setString(2, '%'+status+'%');
			// 4.编译并执行sql语句，并将返回的数据保存到结果集对象
			rs = ps.executeQuery();
			// 5.遍历结果集
			while(rs.next()){
				BookInfo bookInfo = new BookInfo();
				bookInfo.setBid(rs.getInt("b_id"));
				...
				bookInfo.setStatus(rs.getInt("status"));
