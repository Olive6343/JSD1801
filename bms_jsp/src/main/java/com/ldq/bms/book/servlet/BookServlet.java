package com.ldq.bms.book.servlet;
...
import com.ldq.bms.book.service.impl.BookServiceImpl;
...
public class BookServlet extends HttpServlet {
	...
	// 上传文件存储目录
	private static final String UPLOAD_DIRECTORY = "D:\\JSD1801\\liangdiquan\\workspace\\bms_jsp\\src\\main\\webapp\\img\\books";
...
//		List<BookInfo> list = bookService.searchBooksByName(request.getParameter("keywords"));
		List<BookInfo> list = bookService.searchBooksByNameOrStatus(request.getParameter("bname"),request.getParameter("status"));
		String listJson = "{\"total\":" + list.size() + ", \"rows\":" + gson.toJson(list, listType) + "}";
	 ...
	 * @throws IOException
	 */
	private void undercarriage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if (formItems != null && formItems.size() > 0) {
				...
				for (FileItem item : formItems) {
					...
					if (!item.isFormField()) {
						...
						// 将图片名称更新到数据库
						if(request.getParameter("bid") != null && !"".equals(request.getParameter("bid")) && !"undefined".equals(request.getParameter("bid"))){
